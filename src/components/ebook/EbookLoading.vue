  ...
  mounted() {
    this.task = setInterval(() => {
      this.$refs.mask.forEach((item, index) => {
        ...
        mask.style.Width = `${pr(maskWidth.value)}rem`
        mask.style.flex = `0 0 ${pr(maskWidth.value)}rem`
        line.style.Width = `${pr(lineWidth.value)}rem`
        line.style.flex = `0 0 ${pr(lineWidth.value)}rem`
